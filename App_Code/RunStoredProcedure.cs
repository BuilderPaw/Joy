...
public class RunStoredProcedure
{
    ...
    public string GetName(string username)
    {
        ...
        using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["LocalDb"].ConnectionString))
        {
            ...
            {
                // 1.  create a command object identifying the stored procedure
                SqlCommand cmd = new SqlCommand("Proc_GetName", conn);
                ...
                // 3. add parameter to command, which will be passed to the stored procedure
                cmd.Parameters.Add(new SqlParameter("@Username", username));
                cmd.Parameters.Add("@Name", SqlDbType.VarChar, 1000).Direction = ParameterDirection.Output;
            ...
            {
                throw new Exception("Error: GetName method under RunStoredProcedure class: " + ex.Message);
            }
        }
    }
}