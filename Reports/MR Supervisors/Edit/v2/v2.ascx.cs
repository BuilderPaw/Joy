...
public partial class Reports_MR_Supervisors_Edit_v2_v2 : System.Web.UI.UserControl
{
            ...
            if (rdr.HasRows)
            {
                while (rdr.Read())
                {
                    ...
                    if (method.Equals("checkChanges"))
                    {
                        ...
                        if (ReportSupervisorMr.SpecialComments != rdr["SpecialComments"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
    ...
    protected void SetStaticVariable()
    {
        ...
        ReportSupervisorMr.SpecialComments = txtSpecialComments.Text.Replace("\n", "<br />").Replace("'", "^");
    }
}