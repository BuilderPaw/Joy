...
public partial class Reports_MR_Responsible_Gaming_Officer_Print_v1_Active_v1 : System.Web.UI.UserControl
{
            ...
            if (rdr.HasRows)
            {
                while (rdr.Read())
                {
                    if (method.Equals("getFields"))
                    {
                        ...
                        {
                            command.CommandText = "SELECT * FROM [dbo].[List_Location] WHERE [SiteID] = 1 AND [Active] = 1 ORDER BY CASE WHEN [Description] = 'Other' THEN 1 ELSE 0 END, [Description]";
                        ...
                        using (SqlCommand command = new SqlCommand())
                        {
                            command.CommandText = "SELECT * FROM [dbo].[List_RGONotified] WHERE [SiteID] = 1 AND [Active] = 1 ORDER BY CASE WHEN [Description] = 'Other' THEN 1 ELSE 0 END, [Description]";
                            command.Connection = connection;
                            connection.Open();
                            using (SqlDataReader sdr = command.ExecuteReader())
                            {
                        ...
                        using (SqlCommand command = new SqlCommand())
                        {
                            command.CommandText = "SELECT * FROM [dbo].[List_3hrsAlertResponse] WHERE [SiteID] = 1 AND [Active] = 1 ORDER BY CASE WHEN [Description] = 'N/A' THEN 1 ELSE 0 END, [Description]";
                            command.Connection = connection;
                            connection.Open();
                            using (SqlDataReader sdr = command.ExecuteReader())
                            {
