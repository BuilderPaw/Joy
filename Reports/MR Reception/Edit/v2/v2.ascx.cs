...
public partial class Reports_MR_Reception_Edit_v2_v2 : System.Web.UI.UserControl
{
            ...
            if (rdr.HasRows)
            {
                while (rdr.Read())
                {
                    ...
                    if (method.Equals("checkChanges"))
                    {
                        ...
                        if (ReportReceptionMr.SignIn != rdr["SignInSlip"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
                            Report.WhereChangeHappened = "SignInSlip";
                        }
                        if (ReportReceptionMr.Refusals != rdr["Refusals"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
                            Report.WhereChangeHappened = "Refusals";
                        }
                        if (ReportReceptionMr.Events != rdr["EventsField"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
                            Report.WhereChangeHappened = "EventsField";
                        }
                        if (ReportReceptionMr.GenComm != rdr["GeneralComments"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
                            Report.WhereChangeHappened = "GeneralComments";
                        }
                        if (ReportReceptionMr.SpecialComments != rdr["SpecialComments"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
    ...
    protected void SetStaticVariable()
    {
        ...
        ReportReceptionMr.SpecialComments = txtSpecialComments.Text.Replace("\n", "<br />");
    }
}