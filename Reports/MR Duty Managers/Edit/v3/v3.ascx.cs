...
public partial class Reports_MR_Duty_Managers_Edit_v3_v3 : System.Web.UI.UserControl
{
            ...
            if (rdr.HasRows)
            {
                while (rdr.Read())
                {
                    ...
                    if (method.Equals("checkChanges"))
                    {
                        ...
                        if (ReportDutyManagerMr.SpecialComments != rdr["SpecialComments"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
    ...
    protected void SetStaticVariable()
    {
        ...
        ReportDutyManagerMr.SpecialComments = txtSpecialComments.Text.Replace("\n", "<br />").Replace("'", "^");
    }
}