...
public partial class Reports_MR_Reception_Supervisor_Edit_v2_v2 : System.Web.UI.UserControl
{
            ...
            if (rdr.HasRows)
            {
                while (rdr.Read())
                {
                    ...
                    if (method.Equals("checkChanges"))
                    {
                        ...
                        if (ReportReceptionSupervisorMr.SignIn != rdr["SignInSlip"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
                            Report.WhereChangeHappened = "SignInSlip";
                        }
                        if (ReportReceptionSupervisorMr.Refusals != rdr["Refusals"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
                            Report.WhereChangeHappened = "Refusals";
                        }
                        if (ReportReceptionSupervisorMr.Events != rdr["EventsField"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
                            Report.WhereChangeHappened = "EventsField";
                        }
                        if (ReportReceptionSupervisorMr.GenComm != rdr["GeneralComments"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
                            Report.WhereChangeHappened = "GeneralComments";
                        }
                        if (ReportReceptionSupervisorMr.SpecialComments != rdr["SpecialComments"].ToString())
                        {
                            Report.HasChange = true; flag = 1;
    ...
    protected void getChanges()
    {
        ...
        if (Report.HasErrorMessage1 == false)
        {
            Report.ShiftDate = DateTime.Parse(txtDatePicker.Text).ToString();
            Report.ShiftDOW = DateTime.Parse(Report.ShiftDate.ToString()).DayOfWeek.ToString();
        }
    ...
    {
        ReportReceptionSupervisorMr.SignIn = txtSignInSlip.Text.Replace("\n", "<br />");
        ReportReceptionSupervisorMr.Refusals = txtRefusals.Text.Replace("\n", "<br />");
        ReportReceptionSupervisorMr.Events = txtEventsField.Text.Replace("\n", "<br />");
        ReportReceptionSupervisorMr.GenComm = txtGeneralComms.Text.Replace("\n", "<br />");
        ReportReceptionSupervisorMr.SpecialComments = txtSpecialComments.Text.Replace("\n", "<br />");
    }
}