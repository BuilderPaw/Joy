<%@ Control Language="C#" AutoEventWireup="true" CodeFile="v2.ascx.cs" Inherits="Reports_MR_Reception_Supervisor_Print_v2_Active_v2" %>
<div style="font-family: Arial">
    ...
    <h4 style="margin-left: 48.5%; font-family: Arial"><%# (string.IsNullOrWhiteSpace(Eval("ReportName").ToString())) ? Eval("ReportName") : (Eval("ReportName").ToString()).Replace("^", "'") %> ID No. <span style="color: red;"><b><%# (string.IsNullOrWhiteSpace(Eval("ReportId").ToString())) ? Eval("ReportId") : (Eval("ReportId").ToString()).Replace("^", "'") %></b></span></h4>
    <table style="width: 100%; font-family: Arial; border: solid 1px black !important;">
        ...
        <tr>
            <td style="font-size: 12.5px;" colspan="4">
                <%# (string.IsNullOrWhiteSpace(Eval("SpecialComments").ToString())) ? Eval("SpecialComments") : (Eval("SpecialComments").ToString()).Replace("^", "'") %>                                    
            </td>
        </tr>
        <tr>
